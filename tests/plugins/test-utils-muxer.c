...
static const int64_t SEQ_END = -1;
static const int64_t SEQ_AGAIN = -2;
static const int64_t SEQ_PACKET_BEGIN = -3;
static const int64_t SEQ_PACKET_END = -4;
...
static
void do_test(enum test test, const char *name,
...
void test_no_ts(void)
{
	...
	do_test(TEST_NO_TS, "event notifications with no time",
...
void test_no_upstream_connection(void)
{
	...
	do_test(TEST_NO_UPSTREAM_CONNECTION, "no upstream connection",
...
void test_event_notif_vs_the_rest(void)
{
	...
	do_test(TEST_EVENT_NOTIF_VS_THE_REST, "event notifications vs. the rest",
		expected_test_events, true);
}
...
void test_simple_4_ports(void)
{
	...
	do_test(TEST_SIMPLE_4_PORTS, "simple: 4 ports without retries",
		expected_test_events, true);
}
...
void test_4_ports_with_retries(void)
{
	...
	do_test(TEST_4_PORTS_WITH_RETRIES, "4 ports with retries",
		expected_test_events, true);
}
